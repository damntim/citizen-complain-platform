<?php
// Database connection
require_once '../db_setup.php'; // Make sure this sets up $conn as a MySQLi connection
...
// Count active (ongoing) tickets for current week
...
// Count active (ongoing) tickets from last week
...
// Get current and last week's date ranges (you already have this)
$currentWeekStart = date("Y-m-d", strtotime("monday this week"));
$lastWeekStart = date("Y-m-d", strtotime("monday last week"));
$lastWeekEnd = date("Y-m-d", strtotime("sunday last week"));

// Resolved tickets where decision is 'solved' (this week)
...
// Resolved tickets where decision is 'solved' (last week)
...
function getAvgResponseTime($conn, $startDate, $endDate = null) {
    ...
    return $count > 0 ? round($totalSeconds / $count / 3600, 2) : 0; // return hours
}
...
// Fetch recent 5 tickets (you can limit or order as needed)
$sql = "SELECT 
            t.id, 
            t.ticket_number, 
            t.subject, 
...
// Query to fetch recent ticket activities (last 5 for brevity)
$sql = "
    SELECT 
